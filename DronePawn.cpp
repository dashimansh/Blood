...
void ADronePawn::Tick(float DeltaTime)
{
	...
	// 1. Read sensors every frame
	ReadSensors();

	// 2. Apply wind disturbance to velocity
	ApplyWindForce(DeltaTime);

	// 3. Run active flight mode
	if (bLandingMode)
	...
	// 4. Spin propellers based on motor values
	...
	// 7. Auto send MAVLink gains every 5s when tuned
...
// ─────────────────────────────────────────────────────────────
// Manual Flight
// Player input → FC targets → motor outputs → visual tilt
// ─────────────────────────────────────────────────────────────
void ADronePawn::ApplyManualFlight(float DeltaTime)
{
	...
	// Run Flight Controller → get motor outputs
	...
	// This makes drone visually pitch/roll based on
	// what FC commanded to each motor

	// Pitch: rear motors faster → pitch forward
	float PitchDiff = (Motors.BL + Motors.BR)
		- (Motors.FL + Motors.FR);

	// Roll: left motors faster → roll right
	float RollDiff = (Motors.FL + Motors.BL)
		- (Motors.FR + Motors.BR);

	// Scale to tilt angle
	float TargetVisualPitch = FMath::Clamp(
		Input_Pitch * -MaxTiltAngle,
		-MaxTiltAngle, MaxTiltAngle);

	float TargetVisualRoll = FMath::Clamp(
		Input_Roll * MaxTiltAngle,
		-MaxTiltAngle, MaxTiltAngle);

	// Smooth visual tilt
	CurrentPitch = FMath::FInterpTo(
		CurrentPitch, TargetVisualPitch, DeltaTime, 8.f);
	CurrentRoll  = FMath::FInterpTo(
		CurrentRoll,  TargetVisualRoll,  DeltaTime, 8.f);
...
// ─────────────────────────────────────────────────────────────
// Loiter Flight
// FC holds position — wind causes tilt — PID corrects visually
// ─────────────────────────────────────────────────────────────
void ADronePawn::ApplyLoiterFlight(float DeltaTime)
{
	...
	// ── VISUAL TILT FROM PID CORRECTIONS ──
	// When wind pushes drone → PID corrects →
	// motor differential causes visual tilt

	// Pitch differential — rear vs front motors
	float PitchDiff = (Motors.BL + Motors.BR)
		- (Motors.FL + Motors.FR);

	// Roll differential — left vs right motors
	float RollDiff = (Motors.FL + Motors.BL)
		- (Motors.FR + Motors.BR);

	// Scale motor differential to visual tilt angle
	float TargetVisualPitch = FMath::Clamp(
		PitchDiff * MaxTiltAngle * 2.f,
		-MaxTiltAngle, MaxTiltAngle);

	float TargetVisualRoll = FMath::Clamp(
		RollDiff * MaxTiltAngle * 2.f,
		-MaxTiltAngle, MaxTiltAngle);

	// Smooth tilt interpolation
	CurrentPitch = FMath::FInterpTo(
		CurrentPitch, TargetVisualPitch, DeltaTime, 5.f);
	CurrentRoll  = FMath::FInterpTo(
		CurrentRoll,  TargetVisualRoll,  DeltaTime, 5.f);
...
void ADronePawn::ApplyWaypointFlight(float DeltaTime)
{
	...
	if (Dist > WaypointAcceptRadius)
	{
		...
		// Visual pitch forward while flying to waypoint
		CurrentPitch = FMath::FInterpTo(CurrentPitch,
			-SpeedRatio * MaxTiltAngle * 0.6f, DeltaTime, 5.f);
	...
	else
	{
		...
		if (CurrentWaypointIndex < Waypoints.Num() - 1)
		{
			...
			if (GEngine) GEngine->AddOnScreenDebugMessage(
				13, 2.f, FColor::Cyan,
				FString::Printf(
					TEXT(">> WP[%d] REACHED — FLYING TO WP[%d] <<"),
...
void ADronePawn::ApplyMissionFlight(float DeltaTime)
{
	...
	for (int32 i = 0; i < WPs.Num(); i++)
	{
		FColor C = i < CurrentMissionIndex ? FColor::Green
			: i == CurrentMissionIndex ? FColor::Yellow
			: FColor(128, 0, 255);
...
void ADronePawn::ApplyLandingFlight(float DeltaTime)
{
	...
	if (Pos.Z > StopZ + 10.f)
	{
		float DescentSpd = FMath::Clamp(
			(Pos.Z - StopZ) * 0.3f, 30.f, LandingSpeed);
...
// ─────────────────────────────────────────────────────────────
// Spin Propellers
// Motor values drive propeller speeds visually
...
void ADronePawn::SpinPropellers(float DeltaTime)
{
	...
	// CW motors spin negative, CCW positive
...
// ─────────────────────────────────────────────────────────────
// Send Gains to MAVLink → Real Drone
// ─────────────────────────────────────────────────────────────
...
void ADronePawn::RecordFlightPath()
{
	...
	if (FVector::Dist(CP, LastRecordedPos) < PathPointDistance)
		return;
	...
	UInputComponent* PlayerInputComponent)
{
	...
	// Auto Tune — T key hardcoded, no action mapping needed
	PlayerInputComponent->BindKey(
		EKeys::T,
		IE_Pressed,
		this,
		&ADronePawn::OnToggleAutoTune);
}
...
void ADronePawn::OnToggleAutoTune()
{
	if (!FC.bAutoTuning)
	{
		...
		if (GEngine) GEngine->AddOnScreenDebugMessage(
			...
			TEXT(">> AUTO-TUNE STARTED — PRESS T TO STOP <<"));
	}
	else
	{
		FC.StopAutoTune();
		...
		if (GEngine) GEngine->AddOnScreenDebugMessage(
			91, 3.f, FColor::Green,
			TEXT(">> AUTO-TUNE STOPPED — GAINS SAVED <<"));
	}
}
...
void ADronePawn::OnKeyThrottleUp(float Value)
{
	...
	if (Value > 0.f)
		Input_Throttle = FMath::Clamp(
			...
			* GetWorld()->GetDeltaSeconds() * 2.f, -1.f, 1.f);
	else
		Input_Throttle = FMath::Clamp(
			Input_Throttle
			- GetWorld()->GetDeltaSeconds() * 2.f, -1.f, 1.f);
...
void ADronePawn::OnAddWaypoint()
{
	...
	if (GEngine) GEngine->AddOnScreenDebugMessage(
		...
		FString::Printf(
			...
			Waypoints.Num()-1, WP.X, WP.Y, WP.Z));
}
